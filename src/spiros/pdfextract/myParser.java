...
public class myParser extends BaseParser
{
    ...
    public myParser(byte[] input) throws IOException
    {
        super(input);
    }
    ...
    public void parse() throws IOException
    {
        try
        {
            ...
            // sometimes there are some garbage bytes in the header before the
            // header
            ...
            try
            {
                float pdfVersion = Float.parseFloat(header.substring(PDF_HEADER.length(), Math.min(header.length(),
                        PDF_HEADER.length() + 3)));
            ...
            catch (IOException e)
            {
                if (wasLastParsedObjectAnXref)
                {
                    ...
                    // the xref, not sure why the PDF spec allows this but it
                    // does.
                }
                else
                {
    ...
    /**
     * This will skip a header's binary fill bytes. This is in accordance to PDF
     * Specification 1.5 pg 68 section 3.4.1 "Syntax.File Structure.File Header"
     * 
     * @throws IOException
     *             If there is an error reading from the stream.
     */
    ...
    private Object parseObject() throws IOException
    {
        ...
        else if (peekedChar == 'x' || peekedChar == 't' || peekedChar == 's')
        {
            ...
            if (peekedChar == 'x' || peekedChar == 's')
            {
                ...
                skipSpaces();
                // read some integer that is in the stream but PDFBox doesn't
                // use
                readInt();
            }
            ...
            {
                throw new IOException("expected='%%EOF' actual='" + eof + "' next=" + readString() + " next="
                        + readString());
            }
            else if (!pdfSource.isEOF())
            {
                // we might really be at the end of the file, there might just
                // be some crap at the
                // end of the file.
        ...
        else
        {
            ...
            if (endObjectKey.equals("stream"))
            {
                ...
                {
                    // this is not legal
                    // the combination of a dict and the stream/endstream forms
    ...
    /**
     * This will get the document that was parsed. parse() must be called before
     * this is called. When you are done with this document you must call
    ...
    public PushBackInputStream getPdfSource()
    {
        return pdfSource;
    }
}
